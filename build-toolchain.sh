...
res="success"
check_dir_exists "src/gcc" || export res="failure"
check_dir_exists "src/binutils" || export res="failure"
check_dir_exists "src/gdb" || export res="failure"
check_dir_exists "src/newlib" || export res="failure"
check_dir_exists "src/cgen" || export res="failure"
check_dir_exists "$(basename "$sdkdir")" || export res="failure"
...
if [ "${do_gmp}" = "--do-gmp" ]
then
    check_dir_exists "src/gcc-infrastructure/gmp" || export res="failure"
...
if [ "${do_mpfr}" = "--mpfr" ]
then
    check_dir_exists "src/gcc-infrastructure/mpfr" || export res="failure"
    infra_dir="gcc-infrastructure"
else
...
if [ "${do_mpc}" = "--mpc" ]
then
    check_dir_exists "src/gcc-infrastructure/mpc" || export res="failure"
    infra_dir="gcc-infrastructure"
else
...
if [ "${do_isl}" = "--isl" ]
then
    check_dir_exists "src/gcc-infrastructure/isl" || export res="failure"
    infra_dir="gcc-infrastructure"
else
...
if [ "${do_cloog}" = "--cloog" ]
then
    check_dir_exists "src/gcc-infrastructure/cloog" || export res="failure"
    infra_dir="gcc-infrastructure"
else
...
then
    check_dir_exists "src/gcc-infrastructure/ncurses" || export res="failure"
    infra_dir="gcc-infrastructure"
else
...
then
    check_dir_exists "src/gdb-multicore-sim" || export res="failure"
fi
...
then
	...
	--enable-emesh-sim --enable-sim-hardware ${config_extra} \
	>> "${logfile}" 2>&1
    then
...
if `echo ${id_host} | grep ${basedir}/${release_dir} > /dev/null 2>&1` &&
    [ -d "${basedir}/${release_dir}" ]
then
    cd ${basedir}

    if rm -f esdk && ln -s "${release_dir}" esdk
    then
	logterm "Top level ${basedir}/esdk linked to ${basedir}/${release_dir}"
    else
	logterm "Unable to create link to ${basedir}/esdk"
    fi
fi
